...
void Algoritmo::crearClases(){
    ...
    clase = new vector <Solucion> [num_clases];     // Se reserva espacio para las clase y clave.
...
Algoritmo::Algoritmo(const vector <int> & elementos_iniciales, int objetivo){
    ...
    for (int i = 1; i <= num_elementos_iniciales; i++){
        ...
        clase[clave[i]].push_back(elemental);
    }
...
void Algoritmo::resolver(){
    ...
    for (int i = 2; i <= num_elementos_iniciales; i++){ 
        if ( abs(clase[mejor][0].getResultado() - objetivo) > abs(clase[clave[i]][0].getResultado() - objetivo) ){
            mejor = clave[i];
        }
    }
    mejor_solucion = new Solucion(clase[mejor][0]);
...
void Algoritmo::generarSoluciones(int i, int j){
    ...
    int k = i - j;
    Solucion *elemento;
    vector <Solucion>::iterator m;
    vector <Solucion>::iterator n;
